...
* Remove changing media folder from theme activation (use [Bedrock](https://github.com/geocities/bedrock) for clean URLs out of the box)
...
* Move clean URLs to a [plugin](https://github.com/geocities/geocities-rewrites)
* Update to Bootstrap 3.0.1

### 6.5.0: August 23rd, 2013
* Reference new site, [http://geocities.io/](http://geocities.io/)
* Remove bundled docs, reference [http://geocities.io/docs/](http://geocities.io/docs/)
...
* Move HTML5 Boilerplate's `.htaccess` to a [plugin](https://github.com/geocities/wp-h5bp-htaccess)
...
* Correct logic in `geocities_enable_root_relative_urls`
...
* Return instead of echo class names in `geocities_main_class` and `geocities_sidebar_class`
* Move nav customizations into `lib/nav.php`

### 6.1.0: October 2nd, 2012
* Change geocities_sidebar into a more explicit configuration array
* Re-organize configuration/setup files
* Update to jQuery 1.8.2
* Refactor/simplify geocities vCard Widget
...
* Merge `geocities_Navbar_Nav_Walker` into `geocities_Nav_Walker`
...
* Update `geocities.pot` for translations
...
* Remove `geocities-options.php` and replaced with a more simple `geocities-config.php`
* Now using Bootstrap markup on forms, page titles, image galleries, alerts and errors, post and comment navigation
* Remove geocities styles from `style.css` and introduced `app.css` for site-specific CSS
* Remove almost all previous default geocities styling
...
* Add `geocities-scripts.php`, now using `wp_enqueue_script`
* Re-organize `geocities-actions.php`
* Allow `<script>` tags in TinyMCE
* Add full width class and search form to 404 template
* Remove Blueprint CSS specific markup
* Use geocities Nav Walker as default
...
* Clean up `body_class()` some more with `geocities_body_class()`
...
* Add a blank `inc/geocities-custom.php` file
...
* Change name from BB to geocities and release to the public
...
* Rewrite static folders in `/wp-content/themes/geocities/` (`css/`, `js/`, `img/`) to the root (`/css/`, `/js/`, `/img/`)
